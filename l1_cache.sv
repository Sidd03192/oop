...
assign state    = req_valid ? 3'd1 : 3'd0;
...
assign consume_index_req    = start_tag && req_valid;
assign can_accept_index_req = !req_valid || consume_index_req;
assign accept_index_req     = start_index && can_accept_index_req && !mshr_full;

// Stall when the index-stage request slot cannot accept a new request, or
// when all MSHRs are already occupied.
assign l1_stall_out_to_lsq = !can_accept_index_req || mshr_full;
...
//////////////////////////////////////////////////////////////////
    ...
    always_comb begin
        ...
        for (int w = 0; w < L1_WAYS; w++) begin
            if (set_valids[req_index][w] && tags[req_index][w] == incoming_tag) begin
                hit = 1'b1;
...
///////////////////////////////////////////////////////////////////
    // Sequential block - request buffer, install loop, and MSHR/writeback state
    always_ff @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
                    ...
                    if (!(victim_dirty && wb_full && !wb_pop)) begin
                        ...
                        for (int w = 0; w < WORDS_PER_BLOCK; w++) begin
                            if (mshr_store_mask[i][w]) begin
                                install_block[w * DATA_WIDTH +: DATA_WIDTH] =
            ...
            if (consume_index_req) begin
                if (hit) begin
                    if (req_is_write) begin
                        set_contents[req_index][hit_way][req_word_offset * DATA_WIDTH +: DATA_WIDTH] <= req_wdata;
                        set_dirty[req_index][hit_way] <= 1'b1;
                    end
                    lru[req_index] <= ~hit_way[0];
                end else begin
                    mshr_dup = 1'b0;
                    mshr_dup_idx = '0;
                    for (int i = 0; i < NUM_MSHRS; i++) begin
                    ...
                    if (mshr_dup) begin
                        ...
                        mshr_index[mshr_free_idx]      <= req_index;
            ...
            if (accept_index_req) begin
                req_valid       <= 1'b1;
                req_index       <= trace_vaddr[OFFSET_BITS +: INDEX_BITS];
                req_word_offset <= trace_vaddr[OFFSET_BITS-1 -: WORD_BITS];
                req_is_write    <= is_write;
                req_wdata       <= wdata;
            end else if (consume_index_req) begin
                req_valid <= 1'b0;
            end
            ...
            // install loop picks it up on the next cycle.
            for (integer i = 0; i < NUM_MSHRS; i++) begin
