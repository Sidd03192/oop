...
//==============================================================================
// TB_MEMSYS_V2 — Accurate Trace-Driven Testbench for memsys
//==============================================================================
// Purpose: Drive memsys.sv with binary traces from mem-traces-v2/ and verify
//          correct memory operations via a shadow commit model.
//
// Key improvements over tb_memsys_trace.sv:
//   1. wait_all_idle() checks ALL LSQ states, not just {2, 4}
//   2. Named localparams for LSQ/MSHR states instead of magic numbers
//   3. Progress output every N records
//   4. Detailed comments on known RTL bugs
//
// Known RTL Bugs (for reference):
//   [BUG-1] tlb.sv line 75: valid <= ~is_tlb_fill assigns 1 on miss; should be
//           valid <= hit_found && ~is_tlb_fill
//   [BUG-2] l1_cache.sv lines 337,347: duplicate wb_count decrement; dead code
//   [BUG-3] l1_cache.sv line 320: silent miss drop if !mshr_dup && mshr_full
//==============================================================================
...
module tb_memsys_v2;
    ...
    // LSQ state encoding (from lsq.sv)
    localparam logic [1:0] LQ_EMPTY         = 2'd0;
    localparam logic [1:0] LQ_WAITING_ADDR  = 2'd1;
    localparam logic [1:0] LQ_WAITING_ISSUE = 2'd2;

    localparam logic [2:0] SQ_EMPTY        = 3'd0;
    localparam logic [2:0] SQ_UNRESOLVED   = 3'd1;
    localparam logic [2:0] SQ_WAITING_ADDR = 3'd2;
    localparam logic [2:0] SQ_WAITING_DATA = 3'd3;
    localparam logic [2:0] SQ_WAITING_ISSUE = 3'd4;

    // L1/L2 MSHR idle state
    localparam logic [1:0] L1_MS_IDLE = 2'b00;
    localparam logic [1:0] L2_MS_IDLE = 2'b00;

    // Memory model and shadow queue parameters
    ...
    localparam int PROGRESS_INTERVAL   = 500;  // Print progress every N records
    ...
    // Expected memory state (committed stores)
    ...
    // Backing memory (cache line table)
    ...
    //===== Quiescence Check (FIXED) =====
    // KEY FIX: Check ALL LSQ states, not just {2, 4}
    // When trace_truncated=1, LSQ entries waiting for RESOLVEs that will never
    // arrive are expected, so we only require issuable entries (WAITING_ISSUE)
    // to drain plus cache/memory pipelines to be idle.
    task automatic wait_all_idle(
        input integer max_cycles,
        input logic   trace_truncated
    );
        integer waited;
        integer i;
        logic done;
        ...
        begin
            ...
            while (waited < max_cycles && !done) begin
                ...
                for (i = 0; i < 8; i = i + 1) begin
                    if (trace_truncated) begin
                        // Truncated: only check entries that CAN still issue
                        if (dut.u_lsq.lq_state[i] == LQ_WAITING_ISSUE)
                            lsq_quiescent = 1'b0;
                        if (dut.u_lsq.sq_state[i] == SQ_WAITING_ISSUE)
                            lsq_quiescent = 1'b0;
                    end else begin
                        // Full trace: ALL entries must be EMPTY
                        if (dut.u_lsq.lq_state[i] != LQ_EMPTY)
                            lsq_quiescent = 1'b0;
                        if (dut.u_lsq.sq_state[i] != SQ_EMPTY)
                            lsq_quiescent = 1'b0;
                    end
                end
    ...
    always @(posedge clk or negedge rst_n) begin : backing_memory
        ...
        begin
            if (!rst_n) begin
                ...
                // Decrement pending read counters
                for (i = 0; i < READ_PENDING_DEPTH; i = i + 1) begin
    ...
    initial begin
        ...
        begin : drive_trace
            ...
            while ($fread(trace_buffer, trace_fd) == 16) begin
                ...
                // Print progress
                if (records_seen % PROGRESS_INTERVAL == 0)
                    $display("[%0d] records=%0d sent=%0d ops=%0d", cycle_ctr, records_seen, records_sent, mem_ops_seen);
        ...
        $fclose(trace_fd);
        $display("[%0d] Trace done (stopped_by_limit=%0b). Waiting for idle...", cycle_ctr, stopped_by_limit);
        wait_all_idle(timeout_cycles, stopped_by_limit);
        ...
        $display("[%0d] Comparing expected vs actual state...", cycle_ctr);
    ...
    always @(posedge clk) begin
        if (rst_n && cycle_ctr > timeout_cycles) begin
            ...
            $display("========================================");
            $display("  cycle=%0d (limit=%0d)", cycle_ctr, timeout_cycles);
            $display("  records_seen=%0d records_sent=%0d", records_seen, records_sent);
            $display("  stopped_by_limit=%0b reached_eof=%0b", stopped_by_limit, reached_eof);
            $display("  trace_valid=%0b trace_ready=%0b", trace_valid, trace_ready);
            $display("  mem_req_valid=%0b mem_req_is_write=%0b mem_req_addr=0x%0h",
                     mem_req_valid, mem_req_is_write, mem_req_addr);
            $display("  lsq_lq_ready=%0b lsq_sq_ready=%0b lsq_valid_out=%0b",
                     dut.u_lsq.lq_ready, dut.u_lsq.sq_ready, dut.u_lsq.valid_out);
